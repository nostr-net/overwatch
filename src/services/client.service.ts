import { BIG_RELAY_URLS, ExtendedKind, LOCKED_RELAY } from '@/constants'
import {
  compareEvents,
...
class ClientService extends EventTarget {
        ...
        return this._subscribeTimeline(
          ...
          filter,
          {
            onEvents: (_events, _eosed) => {
              ...
              const newEvents = _events.filter((evt) => !eventIdSet.has(evt.id))

              // Use insertManySorted to maintain sorted order efficiently
              events = insertManySorted(events, newEvents, filter.limit)
  ...
  async loadMoreTimeline(key: string, until: number, limit: number) {
    ...
    // Merge all timeline events efficiently using mergeSorted
    let mergedEvents: NEvent[] = []
    for (const timeline of timelines) {
      mergedEvents = mergeSorted(mergedEvents, timeline, limit)
    }
    return mergedEvents
  }
  ...
  private async _subscribeTimeline(
    ...
    const subCloser = this.subscribe(relays, since ? { ...filter, since } : filter, {
      ...
      onevent: (evt: NEvent) => {
        ...
        // new event - log for debugging
        console.log('[ClientService] Event received after EOSE:', {
          eventId: evt.id.slice(0, 8),
          eventCreatedAt: evt.created_at,
          eosedAt,
          isNew: evt.created_at > eosedAt
        })
      ...
      oneose: (eosed) => {
        if (eosed && !eosedAt) {
          eosedAt = dayjs().unix()
          console.log('[ClientService] EOSE received, subscription staying open for new events', {
            eosedAt,
            relays,
        ...
        if (!eosed) {
          // Use updateSortedEvents for efficient sorting and merging
          events = updateSortedEvents([], events, filter.limit)
          const merged = mergeSorted(events, cachedEvents, filter.limit)
          return onEvents([...merged], false)
        }

        // Use updateSortedEvents for efficient sorting
        events = updateSortedEvents([], events, filter.limit)
        const timeline = that.timelines[key]
        // no cache yet
        if (!timeline || Array.isArray(timeline) || !timeline.refs.length) {
  ...
  private async _loadMoreTimeline(key: string, until: number, limit: number) {
    ...
    const remainingLimit = limit - cachedEvents.length
    let events = await this.query(urls, { ...filter, until, limit: remainingLimit })
    events.forEach((evt) => {
      this.addEventToCache(evt)
    })
    // Use updateSortedEvents for efficient sorting
    events = updateSortedEvents([], events, remainingLimit)
    ...
    // Use mergeSorted to efficiently combine cached and new events
    return mergeSorted(cachedEvents, events, limit)
  }

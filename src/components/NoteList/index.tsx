...
import { useDeletedEvent } from '@/providers/DeletedEventProvider.v2'
...
const NoteList = forwardRef(
  (
    ...
    const memoizedSubRequests = useDeepCompareMemo(subRequests)
      ...
      async function init() {
        ...
        const { closer, timelineKey } = await client.subscribeTimeline(
          ...
          })),
          {
              ...
              if (events.length > 0) {
                ...
                if (channelId) {
                  ...
                  events.forEach(event => {
                    channelService.trackEventForUnread(channelId, event.created_at)
                  })
                }
              }
              if (areAlgoRelays) {
            ...
            onNew: (event) => {
              console.log('[NoteList] onNew event received:', {
              ...
              // Track new event for unread calculation
              if (channelId) {
                channelService.updateChannelActivity(channelId)
                channelService.trackEventForUnread(channelId, event.created_at)
              }

              // Auto-load new messages directly into the feed (Slack-style)
              console.log('[NoteList] Adding new event to feed')
              setEvents((oldEvents) =>
    ...
    }, [memoizedSubRequests, refreshCount, showKinds])
